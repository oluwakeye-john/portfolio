                ...
                <div key={index} className="text-center" >
                    <br />
                    {
                        item.url.includes('http')
                            ...
                            : <Link onClick={() => isMenuOpen(false)} activeClassName='menu-active' style={{ color: "white" }} className="menu-item" to={item.url}>
                                <span className={item.icon}> </span> {item.name}
                            </Link>
                            ...
                            <ul className="navbar-nav ml-auto" data-toggle="collapse" data-target=".navbar-collapse.show">
                                {
                                    this.props.menuItems.map((item, index) => (
                                        <div key={index}>
                                            {
                                                item.url.includes('http')
                                                    ? <a href={item.url} style={{ color: this.props.color || "black" }} className="nav-link mr-3" to={item.url}>
                                                        <span className={item.icon}> </span> {item.name}
                                                    </a>
                                                    : <Link style={{ color: this.props.color || "black" }} key={index} activeClassName='menu-active' className="nav-link mr-3" to={item.url}>
                                                        <span className={item.icon}> </span> {item.name}
                                                    </Link>
                                            }
                                        </div>
